...
{ 
	...
	$focus->column_fields['lastname'] = $input_array['firstname'];  
	$focus->column_fields['mailingcity'] = $input_array['mailingcity']; 
	...
	$focus->column_fields['leadsource'] = "Web Site"; 
...
function Update_Contact($input_array){
	...
	$focus->id = $id;
	$focus->mode = 'edit'; 
	$focus->column_fields['lastname'] = $input_array['lastname']; 
	$focus->column_fields['firstname'] = $input_array['firstname'];    
	$focus->column_fields['mailingcity'] = $input_array['mailingcity']; 
	$focus->column_fields['mailingstate'] = $input_array['mailingstate']; 
	$focus->column_fields['mailingcountry'] = $input_array['mailingcountry'];  
	$focus->column_fields['mobile'] = $input_array['mobile']; 
	...
	$output[0]['status'] = true;
	$output[0]['msg'] = "success updated success fully";
	$output[0]['responce'] = $res;
	$output[0]['data'] = $input_array;
	return $output; 
}
...
function get_tickets_list($input_array) {
	...
	for( $j= 0;$j < $noofdata; $j++)
	{
		...
		foreach($fields_list as $fieldlabel => $fieldname) {
			...
			if($fieldname == 'title'){
				$fieldvalue = '<a class="cursor detailView id_'.$ticketid.'"  >'.$fieldvalue.'</a>';
			}
			if($fieldname == 'parent_id') {
				$crmid = $fieldvalue;
				if ($crmid != '') {
					$fieldvalues = getEntityName('Accounts', array($crmid));
					$fieldvalue = '<a class="cursor accountView id_'.$crmid.'"  >'.$fieldvalues[$crmid].'</a>';
				} else {
					$fieldvalue = '';
				}
			}
			if($fieldname == 'contact_id') {
				if(!empty($fieldvalue)) {
					...
					$fieldvalue = '<a class="cursor userView id_'.$crmid.'"  >'.$fieldvalues[$fieldvalue].'</a>';
				} else {
					$fieldvalue = '';
				}
			}
...
function authenticate_user($username,$password,$version,$login = 'true')
{
	...
	$current_date = date("Y-m-d");
	$sql = "select id, user_name, user_password,last_login_time, support_start_date, support_end_date, cryptmode, vtiger_crmentity.label
	...
	$list[0]['label'] = $adb->query_result($result,$i,'label');
	$list[0]['allowed_modules'] = get_modules();
...
function get_ticket_attachments($input_array)
{
	...
	$query = "select vtiger_troubletickets.ticketid, vtiger_crmentity.createdtime, vtiger_crmentity.label,vtiger_attachments.*,vtiger_notes.filename,vtiger_notes.filelocationtype from vtiger_troubletickets " .
	...
	for($i=0;$i<$noofrows;$i++)
	{
		...
		$label = $adb->query_result($res,$i,'label');
...
function add_ticket_attachment($input_array)
{
	...
	$ticketid = $input_array['ticketid'];
	$filename = $input_array['filename'];
	$filename2 = $input_array['filename'];
	...
	$new_filename = $attachmentid.'_'.$filename;
	 ...
	 // move_uploaded_file($filecontents['document']['tmp_name'],'AngularSoap/tmp/'.$new_filename);
	...
	$focus = new Documents();
	$focus->column_fields['notes_title'] = $input_array['title'];
	$focus->column_fields['filename'] = $filename2;
...
function get_list_Project($id,$module,$sessionid,$only_mine='true')
{	
	...
	if($module == "ProjectTask"){
		$query = "SELECT vtiger_project.*, vtiger_projecttask.*, vtiger_crmentity.smownerid
					FROM vtiger_project
					...
					$query = "SELECT vtiger_project . * , vtiger_projectmilestone . * , vtiger_crmentity.smownerid
						FROM vtiger_project
						LEFT JOIN vtiger_crmentity ON vtiger_crmentity.crmid = vtiger_project.projectid
						LEFT JOIN vtiger_projectmilestone ON vtiger_project.projectid = vtiger_projectmilestone.projectid
						WHERE vtiger_crmentity.deleted =0 AND vtiger_project.linktoaccountscontacts IN (".generateQuestionMarks($entity_ids_list).")";
				}

	 

				// 	SELECT vtiger_projecttask.*, vtiger_crmentity.smownerid
	...
	$noofdata = $adb->num_rows($res);
	 ...
	 if($module == "ProjectTask"){
	 $tr = "<tr><th>No</th><th>Name</th><th>Related</th><th>Status</th></tr>";
	 while($row = $adb->fetchByAssoc($res)){ 
            $finaldata[] = $row;
            $tr .= "<tr>
            <td>".$row['projecttask_no']."</td>
            <td><a class='cursor detailView id_".$row['projectid']." '>".$row['projecttaskname']."</td>
            <td>".$row['projectname']."</td>
            <td>".$row['projecttaskstatus']."</td>
            </tr>";
        }
	    ...
	    $tbl .= '<div class="col-md-12">';
         ...
         if($noofdata<0)
		 {
			$list.='<tr><td colspan="6"> Project Task Not Available</td></tr>';
			$tbl .=$list;
				$tbl .='</table> </section></div>';
			return [$tbl];
		 }
 ...
 if($module == "ProjectMilestone"){
 	...
 	// $tbl = $adb->fetchByAssoc($res);
  //  return  [$tbl];
	 $tr = "<tr><th>No</th><th>Name</th><th>Related</th><th>Type</th></tr>";
	 while($row = $adb->fetchByAssoc($res)){ 
            $finaldata[] = $row;
            $tr .= "<tr>
            <td>".$row['projectmilestone_no']."</td>
            <td><a class='cursor detailView id_".$row['projectid']." '>".$row['projectmilestonename']."</td>
            <td>".$row['projectname']."</td>
            <td>".$row['projectmilestonetype']."</td>
            </tr>";
        }
	    ...
	    $tbl .= '<div class="col-md-12">';
         ...
         if($noofdata==0)
		 {
			$list.='<tr><td colspan="6"> Project Milestone Not Available</td></tr>';
			$tbl .=$list;
				$tbl .='</table> </section></div>';
			return [$tbl];
		 }
...
function invoiceList($id,$sessionid){
	...
	$focus = new Invoice();
	$focus->filterInactiveFields("Invoice");
	foreach ($focus->list_fields as $fieldlabel => $values){
		foreach($values as $table => $fieldname){
			$fields_list[$fieldlabel] = $fieldname;
		}
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($module == 'Contacts'){
				if($fieldname == 'lastname' || $fieldname == 'firstname'){
					...
					$fieldvalue ='<a class="cursor userView id_'.$fieldid.'"  >'.$fieldvalue.'</a>';
				}
			}
			if($module == 'Project'){
				if($fieldname == 'projectname'){
					$fieldid = $adb->query_result($res,$j,'projectid');
					$fieldvalue = '<a class="cursor detailView id_'.$fieldid.'">'.$fieldvalue.'</a>';
				}
			}
			if($fieldname == 'entityid' || $fieldname == 'contactid' || $fieldname == 'accountid' || $fieldname == 'potentialid' || $fieldname == 'account' || $fieldname == 'linktoaccountscontacts') {
				...
				if ($crmid != '' && $modulename != '') {
					...
					$fieldvalue = '<a class="cursor orgView id_'.$crmid.'"  >'.$fieldvalues[$crmid].'</a>';
		...
		foreach($fields_list as $fieldlabel =>$fieldname ) {
			...
			if($fieldname == 'entityid' || $fieldname == 'contactid' || $fieldname == 'accountid' || $fieldname == 'potentialid' || $fieldname == 'account' || $fieldname == 'linktoaccountscontacts') {
				...
				if ($crmid != '' && $modulename != '') {
					$fieldvalues = getEntityName($modulename, array($crmid));
					if($modulename == 'Contacts')
					$fieldvalue = '<a class="cursor userView id_'.$crmid.'">'.$fieldvalues[$crmid].'</a>';
					elseif($modulename == 'Accounts')
					$fieldvalue = '<a class="cursor orgView id_'.$crmid.'"  >'.$fieldvalues[$crmid].'</a>';
					elseif($modulename == 'Potentials'){
						$fieldvalue = $adb->query_result($res,$j,'potentialname');
			...
			foreach($fields_list as $fieldlabel=> $fieldname) {
				...
				if($fieldname == 'entityid') {
					...
					if ($crmid != '' && $module != '') {
						...
						if($module == 'Contacts')
						// $fieldvalue = '<a href="index.php?module=Contacts&action=index&id='.$crmid.'">'.$fieldvalues[$crmid].'</a>';
							$fieldvalue = '<a class="cursor userView" id="'.$crmid.'" name="'.$crmid.'">'.$fieldvalues[$crmid].'</a>';
							// $fieldvalue =  $fieldvalues[$crmid];
				...
				// $fieldvalue = '<a href="index.php?module=Products&action=index&productid='.$fieldid.'">'.$fieldvalue.'</a>';
					$fieldvalue = '<a  class="cursor detailView id_'.$fieldid.'" id="'.$fieldid.'" name="sitaram">'.$fieldvalue.'</a>';
...
function get_details($id,$module,$customerid,$sessionid)
{
	...
	// if(!validateSession($customerid,$sessionid))
	// return $id.$module.$customerid.$sessionid;
	...
	if($module == 'Contacts'){
		$record = $adb->query_result($res,0,'crmid');
		$output[0][$module][$nooffields+1]['fieldname'] = "record";
		$output[0][$module][$nooffields+1]['fieldlabel'] = "record";
		$output[0][$module][$nooffields+1]['blockname'] = "record";
		$output[0][$module][$nooffields+1]['fieldvalue'] = $record;
	}
	if($module == 'HelpDesk'){
